...
public class EnumUtil {
  ...
  /**
   * Returns the name (String) of an enum value passed in. Since simply calling name() can cause
   * NullPointerExceptions, this method provides a "null-safe" way to get the name. If null is passed in, null
   * is returned.
   * 
   * @param <T>
   *          the enum class
  ...
  /**
   * Returns the value of the enum class passed in whose name is the same as the String passed in. If an empty
   * or null String is passed in, returns null.
   * 
   * @param <T>
   *          type to be returned
   * @param clazz
   *          class of type to be returned
   * @param value
  ...
  public static <T extends Coded> T getValueByCode(Class<T> clazz, int codedValue) {
    ...
    try {
      ...
      throw new NoSuchElementException(
          "No enum element of type " + clazz + " is coded to value " + codedValue);
    }
  ...
  /**
   * Converts an enum name to a more human-readable description. The result can be handy for drop-down
   * selects, etc. Typically enum values are all caps with an underscore separator (e.g. "MY_ENUM"). The
   * result of this method is a mixed case value with a space separator (e.g. "My Enum").
   * 
   * @param name
   *          enum name
  ...
  /**
   * Examines the passed value and determines whether it can be converted into an enum value of the same type
   * as the defaultValue. If it can, converts it and returns. If not, or if the value is null, returns
   * defaultValue.
   * 
   * @param <T>
   *          type of value to be returned
   * @param defaultValue
   *          returned if unable to convert value
   * @param value
   *          string value to be converted
   * @return Enum represented by value of the same type as defaultValue, or defaultValue if value is null or
   *         cannot be converted
   */
  @SuppressWarnings("unchecked")
