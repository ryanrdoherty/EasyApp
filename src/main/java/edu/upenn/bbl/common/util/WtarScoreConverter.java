...
public class WtarScoreConverter {
  ...
  /**
   * Given the subject's age, converts a raw WTAR score to an FS IQ. This is equivalent to
   * <code>getFsIq(getStandardScore(rawScore, age))</code>.
   * 
   * @param rawScore
   *          raw score
   * @param age
  ...
  public static int getFsIq(int standardScore) {
    ...
    if (fsIq == null) {
      throw new IllegalArgumentException(
          "Standard score passed in (" + standardScore + ") does not have a corresponding FSIQ.");
    }
    return fsIq;
  }
  ...
  private static void validateRawScore(int rawScore) {
    if (rawScore < 0 || rawScore >= _conversionMap.size()) {
      throw new IllegalArgumentException("Raw score passed in (" + rawScore + ") must be between 0 and " +
          (_conversionMap.size() - 1) + " (inclusive).");
    }
  }
