...
/**
 * Abstract class which can serve as a parent to any classes that need to validate input data. (e.g. when
 * processing form input). Typically a child class will want to call the validator methods using a
 * <code>label</code> with the first letter capitalized, since the validation messages generally start with
 * the label.
 * 
 * @author rdoherty
 */
public abstract class DataValidator {
   ...
   * @return true if value is null, or if, after conversion to a String, value contains no characters
   */
  ...
  /**
   * Returns true if the passed value is <= maxLength, or if value is null or empty. Otherwise returns false.
   * If false is returned, an error message is added detailing the validation failure.
   * 
   * @param label
   *          name of the value passed
   * @param value
   *          value to be validated
   * @param maxLength
   *          maximum valid length of the value
   ...
   * Otherwise returns false. If false is returned, an error message is added detailing the validation
   * failure.
   * 
   * @param label
   *          name of the value passed
   ...
   *          value to be validated
   * @param minLength
   *          minimum valid length of the value
   * @param maxLength
   *          maximum valid length of the value
   * @return true if value is blank or valid, else false
   */
    ...
    if (value.length() > maxLength || value.length() < minLength) {
      if (minLength == maxLength) {
        ...
        _validationErrors.add(
            label + " must be between " + minLength + " and " + maxLength + " characters (inclusive).");
      }
      return false;
    }
    return true;
  ...
  /**
   * Returns true if the passed value can be evaluated to be a number, or is null or empty. Otherwise returns
   * false. If false is returned, an error message is added detailing the validation failure.
   * 
   * @param label
   *          name of the value passed
   * @param value
  ...
  /**
   * Returns true if the passed value is null or empty. Otherwise returns false. If true is returned, an error
