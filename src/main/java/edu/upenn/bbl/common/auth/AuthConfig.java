...
/**
 * Holds values used to configure and determine how an <code>Authenticator</code> retrieves access roles and
 * other verification information for applications.
 * 
 * @author rdoherty
 *
 */
public class AuthConfig implements Comparator<AuthConfig>, Comparable<AuthConfig> {
  ...
  public void assertComplete() throws AuthenticationException {
    ...
    if (!_type.equals(Type.DATABASE) || StringUtils.isEmpty(_connectionUrl) ||
        StringUtils.isEmpty(_loginName) || StringUtils.isEmpty(_password)) {
