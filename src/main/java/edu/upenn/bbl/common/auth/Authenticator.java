...
public interface Authenticator {
  ...
  /**
   * Checks the password for the passed user. If valid, returns user attributes and all known roles. If not,
   * returns null. Note that the caller is responsible for encrypting the password using the same algorithm
   * used to store the password originally. No password transformation is performed.
   * 
   * @param username
   *          username to check
   * @param password
  ...
  /**
   * Looks up user attributes <strong>without authenticating</strong>, and returns them. If no user with the
   * passed name can be found, returns null
   * 
   * @param username
   *          user for which to search
  ...
  /**
   * Checks if the given username and password represents a valid user. Same rules as
