...
public class DatabaseAuthenticator implements Authenticator {
  ...
  private static final String GET_UNAUTH_USER_ATTRIBUTES = "select user_name, fname, lname from users where user_name = ?";

  private static final String GET_AUTH_USER_ATTRIBUTES = GET_UNAUTH_USER_ATTRIBUTES + " and user_pass = ?";

  private static final String GET_ROLES_BY_USER_ID = "select role_name from user_roles where user_name = ?";
   ...
   * 
   * @param config
   *          configuration (must be a database configuration)
   * @throws AuthenticationException
   *           if error occurs creating the Authenticator
   */
  ...
  /**
   * Creates a new instance using a DataSource configured in JNDI. Looks up the resource using the passed
   * name.
   * 
   * @param jndiName
   *          JNDI name the desired data source is registered under
   * @throws AuthenticationException
   *           if unable to look up data source
   */
  ...
  private User getUser(String username, String password, boolean authenticate)
      throws AuthenticationException {
  ...
  private Set<String> getAccessRoles(User user) throws AuthenticationException {
    ...
    try {
      ...
      throw new AuthenticationException("Unable to look up access roles for user " + user.getUsername(),
          sqle);
    }
    finally {
      DbUtil.close(rs, ps, conn);
