...
public enum DatabaseType {
  ORACLE(true, "oracle.jdbc.driver.OracleDriver", "oracle.jdbc.pool.OracleDataSource",
      "jdbc:oracle:thin:@{0}:{1}:{2}"),
  MYSQL(true, "com.mysql.jdbc.Driver", "com.mysql.jdbc.jdbc2.optional.MysqlDataSource",
      "jdbc:mysql://{0}:{1}/{2}"),
  POSTGRES(false, "org.postgresql.Driver", "edu.upenn.bbl.common.db.PostgresDataSource",
  ...
  private DatabaseType(boolean queryable, String dbDriverClass, String dataSourceClass,
  ...
  /**
   * @return true if this type of database is queryable using the BBL common querier libraries (typically only
   *         if there exists a JDBC driver for that type)
   */
   ...
   * @param port
   *          port the database is serving on
   * @param sid
   *          SID, or name of the database
