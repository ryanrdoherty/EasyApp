...
/**
 * Factory class that enables construction of a Querier tailored to a specific database type and ready to
 * access a specific data source. Typical usage might be: <code>
...
public class QuerierFactory {
  ...
  /**
   * Generates and returns a Querier specific to the database type and ready to access the DataSource
   * contained in the reference class passed.
   * 
   * @param dsRef
   *          reference to type of and path to database
   * @return tailored Querier
   */
  public static Querier getQuerier(DataSourceReference dsRef) {
    switch (dsRef.getType()) {
      case ORACLE:
        return new OracleQuerier(dsRef);
      case MYSQL:
        return new MySqlQuerier(dsRef);
      default:
