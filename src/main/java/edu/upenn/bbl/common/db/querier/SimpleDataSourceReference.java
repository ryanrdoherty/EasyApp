...
import edu.upenn.bbl.common.db.DataSourceConfig;
import edu.upenn.bbl.common.db.DataSourceFactory;
import edu.upenn.bbl.common.db.DatabaseType;

/**
 * Simple implementation of <code>DataSourceReference</code> which uses a <code>DataSourceConfig</code> to
 * access a specific database via the <code>DataSourceFactory</code>. For use by users who simply want to
 * access the capabilities of the <code>Querier</code> without a custom implementation to configure their
 * <code>DataSource</code> or look up via JNDI.
 * 
 * @author rdoherty
 */
public class SimpleDataSourceReference implements DataSourceReference {
   ...
   * 
   * @param config
   *          database configuration to access
   */
