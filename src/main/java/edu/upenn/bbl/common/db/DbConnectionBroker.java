...
public class DbConnectionBroker implements DataSource, Runnable {
   ...
   * maxCheckoutSeconds: Max time a connection can be checked out before being recycled. Zero value turns
   * option off, default is 60 seconds. debugLevel: Level of debug messages output to the log file. 0 -> no
  ...
  public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns,
      int maxConns, double maxConnTime) throws IOException {

    setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, maxConnTime, false,
   ...
   */
  public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns,
      int maxConns, double maxConnTime, boolean logAppend) throws IOException {

    setupBroker(dbDriver, dbServer, dbLogin, dbPassword, minConns, maxConns, maxConnTime, logAppend,
        DEFAULTMAXCHECKOUTSECONDS, DEFAULTDEBUGLEVEL);
  }
   ...
   */
  public DbConnectionBroker(String dbDriver, String dbServer, String dbLogin, String dbPassword, int minConns,
      int maxConns, double maxConnTime, boolean logAppend, int maxCheckoutSeconds, int debugLevel)
      throws IOException {
    ...
    try {
      for (int i = 1; i < dbLoop; i++) {
        try {
          ...
          if (debugLevel > 0) {
            LOG.info("--->Attempt (" + String.valueOf(i) + " of " + String.valueOf(dbLoop) +
  ...
  /**
   * Housekeeping thread. Runs in the background with low CPU overhead. Connections are checked for warnings
   * and closure and are periodically restarted. This thread is a catchall for corrupted connections and
   * prevents the buildup of open cursors. (Open cursors result when the application fails to close a
   * Statement). This method acts as fault tolerance for bad connection/statement programming.
   */
  @Override
  public void run() {
      ...
      for (int i = 0; i < _currConnections; i++) {
        try {
          ...
          if (_currSQLWarning != null) {
            if (_debugLevel > 1) {
              LOG.info("Warnings on connection " + String.valueOf(i) + " " + _currSQLWarning);
            }
        ...
        try { // Test the connection with createStatement call
          synchronized (_connStatus) {
            if (_connStatus[i] > 0) { // In use, catch it next time!
              ...
              if (_debugLevel > 2) {
                LOG.info("Warning.  Connection " + i + " in use for " + timeInUse + " ms");
              }
              if (maxCheckoutMillis != 0) {
                if (timeInUse > maxCheckoutMillis) {
                  if (_debugLevel > 1) {
                    LOG.info("Warning. Connection " + i + " failed to be returned in time.  Recycling...");
                  }
          ...
          if (_debugLevel > 1) {
            LOG.info(new Date().toString() + " ***** Recycling connection " + String.valueOf(i) + ":");
          }
  ...
  /**
   * This method hands out the connections in round-robin order. This prevents a faulty connection from
   * locking up an application entirely. A browser 'refresh' will get the next connection while the faulty
   ...
   * Finally, if all threads are in use, this method waits 2 seconds and tries again, up to ten times. After
   * that, it returns a null.
   */
  @Override
  public Connection getConnection() {
    ...
    if (_available) {
      ...
      for (int outerloop = 1; outerloop <= 10; outerloop++) {
        ...
        try {
          ...
          if (roundRobin >= _currConnections)
            roundRobin = 0;

          do {
            synchronized (_connStatus) {
              if ((_connStatus[roundRobin] < 1) && (!_connPool[roundRobin].isClosed())) {
    ...
    if (_debugLevel > 2) {
      LOG.info("Handing out connection " + idOfConnection(conn) + " --> " +
  ...
  private void createConn(int i) throws SQLException {
    ...
    LOG.info(
        now.toString() + "  Opening connection " + String.valueOf(i) + " " + _connPool[i].toString() + ":");
  }

  /**
   * Shuts down the housekeeping thread and closes all connections in the pool. Call this method from the
   ...
   * <LI>After <code>millis</code> milliseconds after the shutdown of the housekeeping thread, all connections
   * in the pool are closed.
   * <LI>If any connections were in use while being closed then a <code>SQLException</code> is thrown.
  ...
  /**
   * Less safe shutdown. Uses default timeout value. This method simply calls the <code>destroy()</code>
   * method with a <code>millis</code> value of 10000 (10 seconds) and ignores <code>SQLException</code>
  ...
  /**
   * Ignores the username and password passed in. Once created, this datasource will only connect as a single
   * user.
   */
  @Override
