...
/**
 * This class wraps the information needed to establish a connection or connection pool to a database. Objects
...
public class DataSourceConfig {
  ...
  /**
   * Constructs object using server name, port, and database name (instead of URL). A URL specific to the
   * database type is constructed using these elements.
   * 
   * @param dbType
   *          type of database
   * @param server
   ...
   *          whether this connection should be pooled
   */
  public DataSourceConfig(DatabaseType dbType, String server, int port, String dbName, String user,
      String password, boolean isPooled) {
    init(dbType, server, port, dbName, user, password, isPooled);
  }

  private void init(DatabaseType dbType, String server, int port, String dbName, String user, String password,
      boolean isPooled) {
    try {
      _dbType = dbType;
      _server = server;
      ...
      throw new IllegalArgumentException(
          "Unable to format URL from parts: " + _server + ", " + _port + ", " + _dbName, e);
    }
  }

  /**
   * Constructs object given a set of parameters in a configuration bundle (i.e. properties file). See
   * constants in this class for what values should appear in the properties file. If URL is present, server,
   * port and dbName will be ignored. Pooling setting is not required; default is to create a connection pool.
   * 
   * @param bundleName
  ...
  public DataSourceConfig(String bundleName) {
    ...
    if (!PropertyMapLoader.requiredPropertiesPresent(props, DB_TYPE_KEY, DB_LOGIN_NAME_KEY,
        DB_LOGIN_PASSWORD_KEY)) {
      throw new ConfigurationRuntimeException(
          ">0 required database properties (dbtype, login, password) not present.");
    }
    if (!DatabaseType.isDatabaseType(props.get(DB_TYPE_KEY))) {
      throw new ConfigurationRuntimeException(
          "Database type must be one of " + DatabaseType.getValuesAsString());
    }
    ...
    if (StringUtils.isEmpty(boolStr)) {
      if (!boolStr.equalsIgnoreCase("true") && !boolStr.equalsIgnoreCase("false")) {
        throw new ConfigurationRuntimeException(
            "Database connection pool setting must be (ignoring case) true or false");
      }
    ...
    if (StringUtils.isEmpty(props.get(DB_CONNECTION_URL_KEY))) {
      if (!PropertyMapLoader.requiredPropertiesPresent(props, DB_SERVER_NAME_KEY, DB_SERVER_PORT_KEY,
          DB_NAME_KEY)) {
        throw new ConfigurationRuntimeException(
            "Since URL is absent, >0 required database properties (servername, port, dbname) not present.");
      }
      ...
      init(DatabaseType.valueOf(props.get(DB_TYPE_KEY)), props.get(DB_SERVER_NAME_KEY),
          Integer.parseInt(props.get(DB_SERVER_PORT_KEY)), props.get(DB_NAME_KEY),
          props.get(DB_LOGIN_NAME_KEY), props.get(DB_LOGIN_PASSWORD_KEY), usePooling);
    }
    else {
      init(DatabaseType.valueOf(props.get(DB_TYPE_KEY)), props.get(DB_CONNECTION_URL_KEY),
          props.get(DB_LOGIN_NAME_KEY), props.get(DB_LOGIN_PASSWORD_KEY), usePooling);
    }
  }
  ...
  public boolean equals(Object obj) {
    if (obj instanceof DataSourceConfig) {
      ...
      return (_dbType.equals(dci._dbType) && _url.equals(dci._url) && _user.equals(dci._user) &&
          _password.equals(dci._password) && _isPooled == dci._isPooled);
    }
    return false;
  }
  ...
  @Override
  public int hashCode() {
    return new HashCodeBuilder().append(_dbType).append(_url).append(_user).append(_password).append(
        _isPooled).toHashCode();
  }
