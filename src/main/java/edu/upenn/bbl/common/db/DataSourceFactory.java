 ...
 * A cache of DataSources is maintained and when a DataSource is requested, the cache is consulted before
 * creating a new DataSource. This class is threadsafe and uses DbConnectionBroker as the underlying
...
public class DataSourceFactory {
  ...
  /**
   * Looks up a DataSource in JNDI registered under the passed name and returns it. Note: This assumes the
   * Tomcat implementation of JNDI.
   * 
   * @param jndiName
   *          name with which data source should be looked up
   * @return data source registered through JNDI
   * @throws BBLRuntimeException
  ...
  private static synchronized DataSource getOrCreateDataSource(DataSourceConfig config) throws IOException {
    ...
    if (ds == null) {
      if (config.isPooled()) {
        ds = new DbConnectionBroker(config.getDbType().getDbDriverClass(), config.getUrl(), config.getUser(),
            config.getPassword(), DB_MIN_CONNECTIONS, DB_MAX_CONNECTIONS, DB_MAX_CONNECTION_TIME_MS);
      }
      else {
  ...
  private static DataSource getUnpooledDataSource(DataSourceConfig config) {
    try {
      ...
      Class<? extends DataSource> clazz = (Class<? extends DataSource>) loader.loadClass(
          config.getDbType().getDataSourceClass());
      ...
      throw new RuntimeException("Could instantiate vendor-specific DataSource", e);
    }
