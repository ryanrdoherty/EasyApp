...
public class LoginAction extends BaseAction {
  ...
  public String doWork() throws Exception {
    ...
    if (user == null) {
      LOG.debug(
          "Could not authenticate user:password combination " + getUsername() + ":" + encryptedPassword);
  ...
  /**
   * Allows pages to set the "original" request URL when they go to a login page. By submitting this value
   * with the login form values, they can redirect to the original page once user is logged in. Note that if
   * user originally goes to the login page, a nasty loop occurs; also, POST requests are not supported, so
   * parameters will be lost and a GET to the original page will be performed, which may produce errors
  ...
  /**
   * Overrides BaseAction.getRequestUrl() to return the page originally requested. See
   * <code>setRequestUrl()</code> for limitations.
   * 
