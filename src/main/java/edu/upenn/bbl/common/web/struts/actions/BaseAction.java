...
/**
 * This should be the base class for all actions in Struts2-based BBL web apps. It takes care of a number of
 * things:
 * <ul>
...
public abstract class BaseAction extends ActionSupport implements ServletRequestAware {
  ...
  public class Developer {
    public String getName() {
      return _propertyMap.get("developer.name");
    }

    public String getTitle() {
      return _propertyMap.get("developer.title");
    }

    public String getPhone() {
      return _propertyMap.get("developer.phone");
    }

    public String getEmail() {
      return _propertyMap.get("developer.email");
    }
  }
  ...
  /**
   * Child classes must implement. This is the "work" of the action and will be called by
   * BaseAction.execute().
   * 
  ...
  /**
   * Method called by Struts to initiate action logic. Takes care of user authentication via
   * actionRequiresLogin() and access roles. Simply returns the value returned by doWork();
   * 
   * @return action status
   * @throws Exception
   *           if something goes wrong
  ...
  /**
   * Returns the user currently logged into this application, or null if no one is logged in, if the user is
   * no longer valid, or if an error occurs (the error having been logged).
   * 
   * @return current user
   */
  ...
  /**
   * Returns the set of access roles required to execute this action (and access the associated page). Base
   * class returns an empty set. Child classes are encouraged to override, returning application-specific
   * roles.
   * 
  ...
  /**
   * Returns a map of application properties read from the application.properties file. This allows
   * applications to add their own custom properties to the file and read them at will from actions.
   * 
